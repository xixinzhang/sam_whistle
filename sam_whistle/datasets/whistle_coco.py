...
class WhistlePatchCOCO(WhistleCOCO):
    ...
    def _prepare_balanced_patches(self,):
        ...
        if neg_num > pos_num:
            ...
            patches = np.array(self.patches)[indices] # (N, W, C)
            masks = np.array(self.masks)[indices] # (H, W)
        else:
...
if __name__ == "__main__":
    ...
    data_set = WhistlePatchCOCO(root='data/dclde/cross_coco/train/data', annFile='data/dclde/cross_coco/train/labels.json')
    print(len(data_set))
    iter_train_patch = iter(data_set)
