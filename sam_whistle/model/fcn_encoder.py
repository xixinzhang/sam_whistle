...
class FCN_encoder(nn.Module):
    ...
    def __init__(self, args: Args, img_size = 1024):
        ...
        self.upsample = nn.Sequential(
                ...
                nn.Conv2d(1521, 256, kernel_size=3, padding=1),
        )
    ...
    def forward(self, spect:torch.Tensor):
        ...
        spect_feat = torch.cat(pred_patches, dim=1)  # stacked patch features
        upsampled_feat = self.upsample(spect_feat)
        mask_logits = self.decoder(upsampled_feat)
